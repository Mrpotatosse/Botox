using Botox.Hook;
using Botox.Protocol;
using Botox.Proxy;
using EasyHook;
using SocketHook;
using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.Linq;
using System.Runtime.Remoting;
using System.Text;
using System.Threading.Tasks;

namespace Botox
{
    public class Program
    {
        
        public static void Main(string[] args)
        {
            ProtocolManager.Instance.Get(1);
            string exe = "D:/DofusApp/Dofus.exe";
            string hookPath = "./SocketHook.dll";
            int redirectionPort = ProxyManager.Instance.Port;

            try
            {
                // test link https://louisabraham.github.io/LaBot/decoder.html?hex=&from_client=true
                
                HookManager.Instance.InitHook(exe, hookPath, redirectionPort);
            }
            catch (Exception e)
            {
                Console.WriteLine($"{e}");
            }

            Console.ReadLine();
        }
    }
}
